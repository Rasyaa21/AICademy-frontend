  ...
  <div class="mx-auto space-y-8 max-w-4xl">
    ...
    <template v-else-if="questionnaire">
      ...
      <div v-if="!showResults" class="space-y-8">
        ...
        <div v-if="isLastQuestion" class="p-4 bg-gray-50 rounded-lg border">
          <h4 class="font-medium text-gray-800 mb-2">Debug Info:</h4>
          <pre class="text-xs text-gray-600 whitespace-pre-wrap">{{ debugSubmissionData }}</pre>
        </div>
      </div>
...
import QuestionnaireResultRecommendations from '~/components/dashboard-student/questionnaires/QuestionnaireResultRecommendations.vue'
import QuestionnaireResultActions from '~/components/dashboard-student/questionnaires/QuestionnaireResultActions.vue'
...
const { data: QuestionnaireData, pending, error, refresh } = await useAsyncData<ApiInterface<QuestionnaireData>>(
  ...
  () => $fetch(`/student/questionnaire/active`, {
...
const downloadResult = () => {
  ...
  const content = {
    student_result: results.value,
    generated_at: new Date().toISOString(),
    recommendations: results.value.recommendations || []
  }
  ...
  a.href = url
  a.download = `questionnaire-result-${results.value.id}.json`
