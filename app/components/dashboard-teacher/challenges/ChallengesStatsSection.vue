<template>
    <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
        <div class="p-6 bg-white rounded-xl border shadow-sm">
            <div class="flex justify-between items-center">
                <div>
                    <p class="text-sm text-gray-500">Total Challenge</p>
                    <p class="text-2xl font-bold text-gray-900">{{ challengeStats.total }}</p>
                </div>
                <div class="flex justify-center items-center w-12 h-12 rounded-xl bg-primary/10">
                    <Icon name="heroicons:trophy-20-solid" class="w-6 h-6 text-primary" />
                </div>
            </div>
        </div>

        <div class="p-6 bg-white rounded-xl border shadow-sm">
            <div class="flex justify-between items-center">
                <div>
                    <p class="text-sm text-gray-500">Challenge Aktif</p>
                    <p class="text-2xl font-bold text-green-600">{{ challengeStats.active }}</p>
                </div>
                <div class="flex justify-center items-center w-12 h-12 bg-green-100 rounded-xl">
                    <Icon name="heroicons:play-circle-20-solid" class="w-6 h-6 text-green-600" />
                </div>
            </div>
        </div>

        <div class="p-6 bg-white rounded-xl border shadow-sm">
            <div class="flex justify-between items-center">
                <div>
                    <p class="text-sm text-gray-500">Challenge Selesai</p>
                    <p class="text-2xl font-bold text-blue-600">{{ challengeStats.completed }}</p>
                </div>
                <div class="flex justify-center items-center w-12 h-12 bg-blue-100 rounded-xl">
                    <Icon name="heroicons:check-circle-20-solid" class="w-6 h-6 text-blue-600" />
                </div>
            </div>
        </div>

        <div class="p-6 bg-white rounded-xl border shadow-sm">
            <div class="flex justify-between items-center">
                <div>
                    <p class="text-sm text-gray-500">Total Submission</p>
                    <p class="text-2xl font-bold text-purple-600">{{ challengeStats.totalSubmissions }}</p>
                </div>
                <div class="flex justify-center items-center w-12 h-12 bg-purple-100 rounded-xl">
                    <Icon name="heroicons:document-text-20-solid" class="w-6 h-6 text-purple-600" />
                </div>
            </div>
        </div>
    </div>
</template>

<script setup lang="ts">
interface ChallengeStats {
    total: number
    active: number
    completed: number
    totalSubmissions: number
}

defineProps<{
    challengeStats: ChallengeStats
}>()
</script>