<!-- components/dashboard-student/challange/ChallengeFilter.vue -->
<template>
  <div class="bg-white rounded-xl p-6 shadow-sm border">
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-4">
      <div>
        <h3 class="text-lg font-semibold text-gray-900">Filter & Pencarian</h3>
        <p class="text-sm text-gray-500 mt-1">{{ filteredCount }} dari {{ totalCount }} challenge ditemukan</p>
      </div>
    ...
    <div class="flex flex-col lg:flex-row gap-4">
      ...
      <div class="relative flex-1">
        <Icon name="heroicons:magnifying-glass-20-solid" class="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
        <input
          ...
          class="w-full border border-gray-200 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all duration-200"
        />
      </div>
      ...
      <div class="relative">
        <select
          ...
          @change="updateFilter('selectedStatus', ($event.target as HTMLSelectElement).value)"
      ...
      <button
        ...
        class="px-4 py-2 text-sm font-medium text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-xl transition-colors duration-200 flex items-center gap-2 border border-gray-200"
      >
...
const emit = defineEmits<{
  "update-filter": [key: keyof ChallengeFilters, value: string];
  "clear-filters": [];
}>();
