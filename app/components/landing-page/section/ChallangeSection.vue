...
const filteredChallenges = computed(() => {
  if (selectedCategory.value === "Frontend") {
    return challenges.value.filter((c) => c.title.toLowerCase().includes("react") || c.title.toLowerCase().includes("frontend") || c.title.toLowerCase().includes("portfolio"));
  }
  if (selectedCategory.value === "Cybersecurity") {
    return challenges.value.filter((c) => c.title.toLowerCase().includes("security") || c.title.toLowerCase().includes("ctf") || c.title.toLowerCase().includes("cyber"));
  }
  if (selectedCategory.value === "UI/UX") {
    return challenges.value.filter((c) => c.title.toLowerCase().includes("ui") || c.title.toLowerCase().includes("design") || c.title.toLowerCase().includes("ux"));
  }
  // All
  ...
  <section class="w-full py-20 bg-gradient-to-b from-gray-50 to-white relative overflow-hidden">
    ...
    <div class="relative max-w-7xl mx-auto px-6">
        ...
        <div class="flex items-center space-x-2 mb-4 justify-center">
          <div class="w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center">
            <svg class="w-6 h-6 text-primary" fill="currentColor" viewBox="0 0 24 24">
              <path
                d="M16 11c1.7 0 3-1.3 3-3S17.7 5 16 5s-3 1.3-3 3 1.3 3 3 3zM8 11c1.7 0 3-1.3 3-3S9.7 5 8 5 5 6.3 5 8s1.3 3 3 3zm0 2c-2.3 0-7 1.2-7 3.5V20h14v-3.5C15 14.2 10.3 13 8 13zm8 0c-.3 0-.6 0-.9.1 1 .7 1.9 1.7 1.9 2.9V20h6v-3.5C23 14.2 18.3 13 16 13z"
              />
            </svg>
          </div>
          <span class="text-sm font-semibold text-primary bg-primary/10 px-3 py-1 rounded-full"> Challenge & Collaboration </span>
        </div>

        <h2 class="text-4xl lg:text-5xl font-bold text-secondary mb-4">Jelajahi Semua <span class="text-primary">Challenge</span></h2>
        <p class="text-lg text-gray-600 max-w-3xl mx-auto">Asah kemampuan kamu dengan challenge-challenge menarik yang khusus disediakan oleh guru dan sekolah!</p>
      </div>

      <div class="flex justify-center mb-12">
        <SegmentedControl :options="['All', 'Frontend', 'UI/UX', 'Cybersecurity']" v-model="selectedCategory" />
      </div>

      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-8">
        <ChallengeCard v-for="challenge in filteredChallenges.slice(0, 4)" :key="challenge.id" :challenge="challenge" />
      </div>
      ...
      <div class="text-center mt-16">
        <NuxtLink to="/student/dashboard/challange" class="bg-primary text-white px-8 py-3 rounded-2xl font-semibold hover:bg-primary/90 transition-colors shadow-lg inline-block"> Lihat Semua Challenge </NuxtLink>
      </div>
    </div>
  </section>
</template>
