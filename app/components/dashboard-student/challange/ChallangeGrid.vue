  ...
  <div>
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
      <div class="flex items-center gap-2 text-sm text-gray-600">
        ...
        <select :value="sortBy" @change="$emit('update-sort', ($event.target as HTMLSelectElement).value)" class="bg-transparent border-none text-primary font-medium cursor-pointer focus:outline-none">
      ...
      <div class="flex items-center gap-2">
        ...
        <button @click="$emit('update-view-mode', 'grid')" :class="['p-2 rounded-lg transition-colors', viewMode === 'grid' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']">
          <Icon name="heroicons:squares-2x2-20-solid" class="w-4 h-4" />
        </button>
        <button @click="$emit('update-view-mode', 'list')" :class="['p-2 rounded-lg transition-colors', viewMode === 'list' ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200']">
    ...
    <div v-if="challenges.length > 0">
      ...
      <div v-else class="space-y-4">
        <ChallengeListItem v-for="challenge in challenges" :key="challenge.id" :challenge="challenge" />
      </div>
    </div>
    ...
    <div v-else class="text-center py-12">
      ...
      <p class="text-gray-500 mb-4">Coba ubah filter untuk melihat challenge lainnya</p>
      <button @click="$emit('clear-filters')" class="text-primary hover:text-primary/80 font-medium">Reset semua filter</button>
    </div>
  </div>
</template>
