...
<template>
  <div class="space-y-6">
    ...
    <!-- Challenge Display -->
    <ChallangeDisplaySection v-if="filteredChallenges.length > 0" :view-mode="viewMode" :paginated-challenges="paginatedChallenges" />
    ...
    <div v-else class="py-12 text-center">
      ...
      <p class="mb-4 text-gray-500">{{ searchQuery ? "Coba ubah kata kunci pencarian atau" : "Coba ubah" }} filter untuk melihat challenge lainnya</p>
    ...
    <div v-if="filteredChallenges.length > itemsPerPage" class="flex justify-center">
      <nav class="flex gap-2 items-center">
        <button @click="currentPage--" :disabled="currentPage === 1" class="px-3 py-2 text-sm font-medium rounded-lg border border-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
          Previous
        </button>

        <span class="px-4 py-2 text-sm text-gray-600"> Page {{ currentPage }} of {{ totalPages }} </span>

        <button @click="currentPage++" :disabled="currentPage === totalPages" class="px-3 py-2 text-sm font-medium rounded-lg border border-gray-200 transition-colors disabled:opacity-50 disabled:cursor-not-allowed hover:bg-gray-50">
          Next
        </button>
      </nav>
    </div>
...
const filteredChallenges = computed(() => {
  ...
  if (searchQuery.value) {
    ...
    filtered = filtered.filter((challenge) => challenge.title.toLowerCase().includes(query) || challenge.description.toLowerCase().includes(query) || challenge.organizer.toLowerCase().includes(query));
  }
