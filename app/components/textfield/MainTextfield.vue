...
/**
 * Tampilkan selalu sebagai string di input,
 * tapi saat type==='number' emit bilangan (number).
 */
...
function onInput(e: Event) {
  ...
  if (props.type === 'number') {
    // kalau kosong, pertahankan kosong agar user bisa clear input tanpa jadi NaN
    if (raw === '') {
      emit('update:modelValue', '' as unknown as string) // tetap kosong
      return
    }
    // gunakan valueAsNumber agar “1e2”, “001” dll diparse benar oleh browser
...
const inputType = computed(() => {
  if (props.isPassword) return isObscure.value ? 'password' : 'text'
  return props.type
})

function toogleObscure() {
  if (props.isPassword) isObscure.value = !isObscure.value
}

const inputId = computed(
  () => props.id || props.name || `tf-${Math.random().toString(36).slice(2)}`
)
</script>

<template>
  <div class="space-y-2">
    <label v-if="label" :for="inputId" class="text-sm font-semibold text-gray-700 block">
      {{ label }}
    </label>
